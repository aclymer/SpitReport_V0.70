	...
	Pebble.sendAppMessage(message, function(e) 
						{
							...
							if (DEBUG === true) console.log("Unable to deliver; transactionId="+e.data.transactionId+" Error is: "+e.error.message);
							delivered = false;
						});
	return delivered;
}
...
Pebble.addEventListener("ready",
						function(e) {
							...
							if (e.type == "ready") navigator.geolocation.watchPosition(locationSuccess, locationError, locationOptions);
						});
Pebble.addEventListener("appmessage",
						function(e) {
							...
							{
								navigator.geolocation.getCurrentPosition(fetchSurf(spot_id, e.payload.index), locationError, locationOptions);
							}
							else if (e.payload.index == "return")
							{
								fetchSurf(nearby_spot_id, e.payload.index);
							}
							else if (e.payload.index == "next" || e.payload.index === "refresh" || e.payload.index ==="prev")
							{
								fetchSurf(spot_id, e.payload.index);
							}
							else if (e.payload.index === "above" || e.payload.index === "below")
							{
								fetchNeighbor(spot_id, e.payload.index);
								fetchSurf(spot_id, e.payload.index);
							}
							else sendMessage({"ready_key": "ready"});
							});